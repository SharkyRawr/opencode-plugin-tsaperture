...
test("filters out models without ids and dedupes duplicates", async () => {
  ...
  assert.deepEqual(
    ...
    ["dupe"],
    "dupes collapse and blank-id models are dropped",
  );
  assert.deepEqual(
...
test("registers models in degraded mode when /api/providers fails", async () => {
  const config = await createConfig({
    "aperture.example/api/providers": () =>
    ...
    "aperture.example/v1/models": models([
      {
        ...
        metadata: {
          provider: { id: "openai-sub", name: "OpenAI (Subscription)" },
        },
      },
    ]),
  });
  ...
  assert.equal(
    config.provider["aperture-openai-subscription"].models["gpt-5.5"].name,
    "gpt-5.5",
  );
});
...
test("surfaces a /v1/models HTTP error as a startup failure", async () => {
  ...
  try {
            ...
            log: async ({ body }) => {
              ...
              if (
                body.message.includes(
                  "Startup step Aperture model discovery finished",
                )
              )
                discoveryFinished();
              return {};
    ...
    assert.equal(
      config.provider,
      undefined,
      "no provider config is emitted on failure",
    );
    assert.equal(
      toasts.length,
      2,
      "the queue continues draining after a rejected toast",
    );
    assert.ok(
      logs.some(
        (entry) =>
          entry.level === "warn" &&
          entry.message.includes("Failed to show opencode toast"),
      ),
    );
  } finally {
    restore();
  ...
  assert.ok(
    toasts.some(
      (t) =>
        t.variant === "error" && t.message.includes("Failed to fetch models"),
    ),
